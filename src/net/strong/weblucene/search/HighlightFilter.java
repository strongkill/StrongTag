 ...
 * 
 * <ul>
 * <li> StopFilter removed tokens from token stream according to stop words
 * table; </li>
 * <li> HighlightFilter only allow words return from the input TokenStream that
 * are named in the array of words. </li>
 * </ul>
 * 
 * 
 * @author Che, Dong
...
public final class HighlightFilter extends TokenFilter {
	...
	public static final ArrayList makeStopTable(String[] wordList) {
		ArrayList<String> stopTable = new ArrayList<String>(wordList.length);
	...
	public final Token next() throws IOException {
		...
		for (Token token = input.next(); token != null; token = input.next()) {
			...
			if (tokenTextBuffer.length() == 0) {
				...
				/**
				 * find next token: <br>
				 * if current token overlaped with previous token: C1C2 C2C3 C2
				 * with same C2 <br>
				 * then merged with cached privious token
				 */
				if (wordList.indexOf(token.termText()) != -1) {
					if (token.startOffset() > (currentEnd)) {
						Token returnToken = new Token(tokenTextBuffer
