				...
				if (null != defMethod) // static POJO
					...
					try {
						...
						throw Lang.makeThrow(	"Entity [%s] is invailid, it should has at least one of:"
														+ " \n1. %s \n2.%s \n3. %s \n4. %s, \n(%s)",
												type.getName(),
												"Accessable constructor with one parameter type as java.sql.ResultSet, another parameter type is net.strong.dao.FiledMatcher(optional)",
												"Accessable static method with one parameter type as java.sql.ResultSet, another parameter type is net.strong.dao.FiledMatcher(optional) and return type is ["
														+ type.getName()
														+ "]",
