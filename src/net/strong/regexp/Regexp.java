...
public final class Regexp
{
	...
	public static String doFilter(Servlet servlet,String src_txt){
		if(words.size()==0 || updateTime-System.currentTimeMillis() > 1800*1000 )
		{
			initRegExpWords(servlet);
		}
		Entry entry ;
		for(Iterator it = words.entrySet().iterator();it.hasNext(); ){
			entry = (Entry)it.next();
			src_txt.replaceAll((String)entry.getKey(), (String)entry.getValue());
		}
		//
		return src_txt;
	}	
	public static String doFilter(PageContext pageContext,String src_txt){
		if(words.size()==0 || updateTime-System.currentTimeMillis() > 1800*1000 )
		{
			initRegExpWords(pageContext);
		}
		Entry entry ;
		for(Iterator it = words.entrySet().iterator();it.hasNext(); ){
			entry = (Entry)it.next();
	...
	public static synchronized void initRegExpWords(Servlet servlet){
		words.clear();
		updateTime = System.currentTimeMillis();
		Connection con = null;
		try{
			con = DriverManager.getConnection(Constants.getProxool_ReadOnly_Pools(servlet));
			ResultSet rs = con.createStatement().executeQuery("select bg_regexp,bg_replace from blog_regexp order by id desc");
			while(rs.next()){
				words.put(rs.getString("bg_regexp"), rs.getString("bg_replace"));
			}
		}catch(Exception e){
			e.printStackTrace();
		}finally{
	...
	public static synchronized void initRegExpWords(PageContext pageContext){
		words.clear();
		updateTime = System.currentTimeMillis();
		Connection con = null;
		try{
